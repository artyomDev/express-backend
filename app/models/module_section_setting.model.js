...
SectionSettingModule.create = async (newSectionSetting, result) => {
  try {
    const [res, fields] = await sql.promise().query(
      "INSERT INTO module_section_setting SET internalName = ?, width = ?, maxWidth = ?, height = ?, maxHeight = ?, backgroundColor = ?, backgroundImageId = ?, borders = ?, borderStyle = ?, borderThickness = ?, borderColor = ?, padding = ?, margin = ?, createdAt = NOW(), updatedAt = NOW()", 
      [newSectionSetting.internalName, newSectionSetting.width, newSectionSetting.maxWidth, newSectionSetting.height, newSectionSetting.maxHeight, newSectionSetting.backgroundColor, newSectionSetting.backgroundImage ? newSectionSetting.backgroundImage.id : null, newSectionSetting.borders, newSectionSetting.borderStyle, newSectionSetting.borderThickness, newSectionSetting.borderColor, newSectionSetting.padding, newSectionSetting.margin]
    );
...
SectionSettingModule.updateById = async (sectionSettingId, sectionSetting, result) => {
  try {
    const [res, fields] = await sql.promise().query(
      "UPDATE module_section_setting SET internalName = ?, width = ?, maxWidth = ?, height = ?, maxHeight = ?, backgroundColor = ?, backgroundImageId = ?, borders = ?, borderStyle = ?, borderThickness = ?, borderColor = ?, padding = ?, margin = ? WHERE sectionSettingId = ?",
      [sectionSetting.internalName, sectionSetting.width, sectionSetting.maxWidth, sectionSetting.height, sectionSetting.maxHeight, sectionSetting.backgroundColor, sectionSetting.backgroundImage ? sectionSetting.backgroundImage.id : null, sectionSetting.borders, sectionSetting.borderStyle, sectionSetting.borderThickness, sectionSetting.borderColor, sectionSetting.padding, sectionSetting.margin, sectionSettingId]
    );
